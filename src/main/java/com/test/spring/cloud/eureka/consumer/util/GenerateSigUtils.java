...
public class GenerateSigUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(GenerateSigUtils.class);
     ...
     * 生成签名信息
     * @param secretKey 产品私钥
     * @param params 接口请求参数名和参数值组成的tuple
     * @return
     * @throws UnsupportedEncodingException
     */
    public static String genSignature(String secretKey, Tuple<String, String>... params) throws UnsupportedEncodingException {
        // 1. 参数名按照ASCII码表升序排序
        Arrays.sort(params);
        // 2. 按照排序拼接参数名与参数值
        StringBuffer paramBuffer = new StringBuffer();
        for (Tuple<String, String> tuple : params) {
            paramBuffer.append(tuple.getFirst()).append(tuple.getSecond() == null ? "" : tuple.getSecond());
        }
        // 3. 将secretKey拼接到最后
        paramBuffer.append("secretKey").append(secretKey);
        // 4. MD5是128位长度的摘要算法，用16进制表示，一个十六进制的字符能表示4个位，所以签名后的字符串长度固定为32个十六进制字符。
